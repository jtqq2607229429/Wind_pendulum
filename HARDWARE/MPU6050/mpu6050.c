...
/**************************************************************************
函数功能：读取MPU6050初始稳定时的角度
入口参数：无
返回  值：摄氏温度
作    者：平衡小车之家
**************************************************************************/

void Read_AngleZero(int flag,float standard)
{
	if(flag)
	{
		if(abs(gyro[0])<standard&&abs(gyro[1])<standard) 
		{
			count_Zero++;
	    	if(count_Zero>=20) ZHONGZHI_A=pitch,ZHONGZHI_B=roll,Flag_Zero=0;
		}
		else count_Zero=0;
	}
